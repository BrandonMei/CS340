...
page = requests.get('https://calbears.com/roster.aspx?roster=369&path=baseball', headers={'User-Agent': 'Custom'})
html = page.text
...
for item in name:
    newurl = "https://calbears.com/" + item.a.get("href")
    ...
    image = "https://calbears.com/" + newsoup.find("div", "sidearm-roster-player-image").find("img")["src"]
    name = firstName + "_" + lastName + ".jpg"
    imageFile = requests.get(image, headers={'User-Agent': 'Custom'}, stream=True)
    output = open("CAL/image/" + name, "wb")
