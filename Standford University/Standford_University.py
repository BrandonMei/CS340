...
page = requests.get('https://gostanford.com/roster.aspx?roster=614&path=bsb', headers={'User-Agent': 'Custom'})
html = page.text
...
for item in name:
    newurl = "https://gostanford.com/" + item.a.get("href")
    ...
    image = "https://gostanford.com/" + newsoup.find("div", "sidearm-roster-player-image").find("img")["src"]
    name = firstName + "_" + lastName + ".jpg"
    imageFile = requests.get(image, headers={'User-Agent': 'Custom'}, stream=True)
    output = open("SU/image/" + name, "wb")
