...
page = requests.get('https://thesundevils.com/schedule.aspx?schedule=1450', headers={'User-Agent': 'Custom'})
html = page.text
...
for item in name:
    newurl = "https://thesundevils.com/" + item.a.get("href")
    ...
    image = "https://thesundevils.com/" + newsoup.find("div", "sidearm-roster-player-image").find("img")["src"]
    name = firstName + "_" + lastName + ".jpg"
    imageFile = requests.get(image, headers={'User-Agent': 'Custom'}, stream=True)
    output = open("ASU/image/" + name, "wb")
