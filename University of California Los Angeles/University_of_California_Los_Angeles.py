...
page = requests.get('https://uclabruins.com/roster.aspx?roster=1203&path=baseball', headers={'User-Agent': 'Custom'})
html = page.text
...
for item in name:
    newurl = "https://uclabruins.com/" + item.a.get("href")
    ...
    image = "https://uclabruins.com/" + newsoup.find("div", "sidearm-roster-player-image").find("img")["src"]
    name = firstName + "_" + lastName + ".jpg"
    imageFile = requests.get(image, headers={'User-Agent': 'Custom'}, stream=True)
    output = open("UCLA/image/" + name, "wb")
