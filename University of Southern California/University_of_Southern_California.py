...
page = requests.get('https://usctrojans.com/roster.aspx?path=baseball', headers={'User-Agent': 'Custom'})
html = page.text
...
for item in name:
    newurl = "https://usctrojans.com/" + item.a.get("href")
    ...
    image = "https://usctrojans.com/" + newsoup.find("div", "sidearm-roster-player-image").find("img")["src"]
    name = firstName + "_" + lastName + ".jpg"
    imageFile = requests.get(image, headers={'User-Agent': 'Custom'}, stream=True)
    output = open("USC/image/" + name, "wb")
