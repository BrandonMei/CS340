...
page = requests.get('https://arizonawildcats.com/roster.aspx?roster=3268&path=baseball', headers={'User-Agent': 'Custom'})
html = page.text
...
for item in name:
    newurl = "https://arizonawildcats.com/" + item.a.get("href")
    ...
    image = "https://arizonawildcats.com/" + newsoup.find("div", "sidearm-roster-player-image").find("img")["src"]
    name = firstName + "_" + lastName + ".jpg"
    imageFile = requests.get(image, headers={'User-Agent': 'Custom'}, stream=True)
    output = open("UOA/image/" + name, "wb")
