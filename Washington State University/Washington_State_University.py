...
page = requests.get('https://wsucougars.com/roster.aspx?path=baseball', headers={'User-Agent': 'Custom'})
html = page.text
...
for item in name:
    newurl = "https://wsucougars.com/" + item.a.get("href")
    ...
    image = "https://wsucougars.com/" + newsoup.find("div", "sidearm-roster-player-image").find("img")["src"]
    name = firstName + "_" + lastName + ".jpg"
    imageFile = requests.get(image, headers={'User-Agent': 'Custom'}, stream=True)
    output = open("WSU/image/" + name, "wb")
