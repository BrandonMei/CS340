...
page = requests.get('https://goducks.com/roster.aspx?roster=240&path=baseball', headers={'User-Agent': 'Custom'})
html = page.text
...
for item in name:
    newurl = "https://goducks.com/" + item.a.get("href")
    ...
    image = "https://goducks.com/" + newsoup.find("div", "sidearm-roster-player-image").find("img")["src"]
    name = firstName + "_" + lastName + ".jpg"
    imageFile = requests.get(image, headers={'User-Agent': 'Custom'}, stream=True)
    output = open("UFO/image/" + name, "wb")
